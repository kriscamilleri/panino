<template>
    <div>
      ...
      <div v-if="isFolder"
        ...
        <span class="font-semibold flex-grow truncate"
      ...
      <div v-else
        ...
        <span class="flex-grow truncate"
              :data-testid="`tree-item-file-name-${item.id}`">{{ item.name }}</span>
      ...
      <ul v-if="isFolder && isOpen" class="ml-6 mt-1 border-l pl-2">
        <template v-if="isFiltered">
          ...
          <li v-if="matchingFiles.length===0 && children.some(c=>c.type==='file')">
            ...
            <TreeItem v-for="child in children.filter(c=>c.type==='file')"
                      :key="child.id"
                      :item="child"
                      :is-filtered="true"/>
          </li>
