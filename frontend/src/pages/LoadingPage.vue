...
onMounted(async () => {
  try {
    // Show status messages to inform the user about the loading process
    ...
    if (authStore.isAuthenticated && authStore.user?.name !== 'guest') {
      statusMessage.value = 'Syncing with remote server...'
      await docStore.syncStore.oneTimePull()
      
      // The syncStore will automatically load sync preference
      // based on the localStorage or enable it by default
      // when authenticated and not in guest mode
    }
    
    statusMessage.value = 'Loading your documents...'
    // Ensure structure is loaded properly
    await docStore.structureStore.loadStructure()
    
    // If we get here, the setup is complete
    router.replace('/')
  } catch (error) {
