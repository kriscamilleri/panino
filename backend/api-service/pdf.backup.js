...
/**
 * ============================================================================
 * PDF GENERATION SERVICE - ARCHITECTURE OVERVIEW
 * ============================================================================
 * 
 * PURPOSE:
 * Converts HTML/CSS content to PDF with support for pagination, headers/footers,
 * page numbers, and embedded images. Used for exporting documents as PDFs.
 * 
 * HIGH-LEVEL FLOW:
 * 1. Client POSTs { htmlContent, cssStyles, printStyles } to /render-pdf
 * 2. Request is queued (single-threaded processing to manage Puppeteer memory)
 * 3. HTML is sanitized via DOMPurify
 * 4. All images (internal + external) are converted to base64 data URIs
 * 5. A full HTML document is constructed with CSS, Google Fonts, and CSP headers
 * 6. Puppeteer loads the HTML in a headless Chrome browser
 * 7. Paged.js library is injected to handle CSS pagination (@page rules)
 * 8. Custom headers/footers are injected as DOM overlays on each page
 * 9. Puppeteer's page.pdf() generates the final PDF buffer
 * 10. PDF is returned to client
 * 
 * KEY COMPONENTS:
 * 
 * [1] IMAGE EMBEDDING (Lines ~35-240)
 *     - getImageAsDataUri(): Fetches internal images from disk → base64
 *     - fetchExternalImageAsDataUri(): Fetches external URLs → base64 (with timeout)
 *     - embedAllImagesAsDataUri(): Orchestrates both, removes failed images
 *     WHY: Puppeteer can't fetch from localhost (same-server issue), and external
 *          images may timeout/fail causing Paged.js errors. Pre-embedding solves both.
 * 
 * [2] SINGLETON BROWSER (Lines ~245-280)
 *     - Single Puppeteer browser instance reused across requests
 *     - New browser contexts per request for isolation
 *     WHY: Launching Chrome is expensive (~2-3 seconds). Singleton saves time.
 * 
 * [3] REQUEST QUEUE (Lines ~285-315)
 *     - Sequential processing, one PDF at a time
 *     WHY: Puppeteer is memory-intensive. Parallel requests can crash Node.
 * 
 * [4] PDF GENERATION (Lines ~325+)
 *     a) Construct full HTML with:
 *        - @page CSS rules (size, margins)
 *        - Google Fonts via <link>
 *        - Content Security Policy for security
 *        - User's CSS styles
 *        - HTML content with embedded images
 *     
 *     b) Puppeteer rendering:
 *        - Create browser context + page
 *        - Set print media type
 *        - Load HTML content
 *        - Inject Paged.js from CDN
 *     
 *     c) Paged.js pagination:
 *        - Paged.js reads the DOM and @page CSS
 *        - Splits content into discrete pages (.pagedjs_page elements)
 *        - Handles page breaks, orphans/widows, multi-column, etc.
 *        - Runs inside page.evaluate() with timeout/progress monitoring
 *     
 *     d) Header/Footer injection:
 *        - After Paged.js creates pages, we inject overlay divs
 *        - Supports %p (current page) and %P (total pages) placeholders
 *        - Configurable show/hide on first/last pages
 *        - Positioned absolutely relative to each .pagedjs_page
 *     
 *     e) PDF generation:
 *        - page.pdf() with preferCSSPageSize: true
 *        - Puppeteer respects Paged.js page structure
 * 
 * PRINTSTYLES OPTIONS (from frontend):
 * - pageSize: 'A4', 'Letter', etc.
 * - pageMargin: CSS margin value for left/right margins
 * - headerHeight/footerHeight: Margin top/bottom (reserved for header/footer)
 * - printHeaderHtml/printFooterHtml: HTML templates with %p/%P placeholders
 * - headerFontSize/footerFontSize, headerFontColor/footerFontColor, headerAlign/footerAlign
 * - hideHeaderOnFirst/hideFooterOnFirst, hideHeaderOnLast/hideFooterOnLast
 * - headerOffset/footerOffset: Fine-tune positioning
 * - googleFontFamily: Comma-separated Google Font names
 * - pagedJsUrl: CDN URL for Paged.js (defaults to unpkg)
 * 
 * KNOWN COMPLEXITIES & POTENTIAL ISSUES:
 * 
 * 1. Paged.js is a complex library that re-renders the entire DOM. It can:
 *    - Be slow with large documents or many images
 *    - Fail silently or hang on malformed HTML/CSS
 *    - Have compatibility issues with certain CSS (flexbox in running headers, etc.)
 * 
 * 2. Image handling has multiple fallback layers:
 *    - Regex-based URL parsing (fragile, can miss edge cases)
 *    - External fetch with timeout (network-dependent)
 *    - Failed images are removed (may surprise users)
 * 
 * 3. Header/Footer injection is DOM manipulation after Paged.js:
 *    - Uses absolute positioning + z-index
 *    - Relies on .pagedjs_page structure existing
 *    - Fallback for non-Paged.js rendering (single page)
 * 
 * 4. Multiple timeout layers:
 *    - External image fetch: 5s
 *    - Paged.js rendering: 60s
 *    - Puppeteer default: 90s
 *    - Content loading: 30s
 *    These can cause confusing failures if any layer times out first.
 * 
 * 5. Console logging for debugging is extensive but mixed with production code.
 * 
 * SIMPLIFICATION OPPORTUNITIES:
 * - Consider removing Paged.js if simple pagination suffices (Puppeteer handles basic @page)
 * - Header/footer could use Puppeteer's native headerTemplate/footerTemplate
 * - Image embedding could be simplified with a single processing pass
 * - Queue could use a proper job queue library (bull, bee-queue)
 * 
 * ============================================================================
 */
